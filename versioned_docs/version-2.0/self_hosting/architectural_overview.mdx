...
:::note
LangSmith Self-Hosted will bundle these datastores by default, but you can configure LangSmith to use external datastores.
In a production setting, we _strongly recommend using external datastores_.
:::
...
Postgres is used as the primary data store for transactional workloads and operational data (almost everything besides traces and feedback).
