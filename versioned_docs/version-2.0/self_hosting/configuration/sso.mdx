...
- You will need to provide the `oauthClientId`, `oauthClientSecret`, `langsmithUrl`, and `oauthIssuerUrl` in your `values.yaml` file. This is where you will configure your LangSmith instance.

<CodeTabs
  tabs={[
    HelmBlock(`config:
  authType: mixed
  langsmithUrl: https://langsmith.example.com
  oauth:
    enabled: true
    oauthClientId: \<YOUR CLIENT ID\>
