...
:::note
**Reminder: API keys prefixed with `ls__` will be disabled in favor of `lsv2...` style keys as of LangSmith Helm release v0.7 to be released on or after July 1, 2024.** For more information see [the Admin concepts guide.](https://docs.smith.langchain.com/concepts/admin#api-keys)
:::
...
- v0.1.20 or greater of the `langchainhub` sdk package is required when interacting with the LangSmith Hub prompt repository.
...
- Added global setting to disable creation of new Organizations. See [insert link here] for more details.
