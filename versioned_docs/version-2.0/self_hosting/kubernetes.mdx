...
For the most up-to-date information on deploying to kubernetes/upgrade instructions, please refer to the README in the [LangSmith Helm Chart](https://github.com/langchain-ai/helm/blob/main/charts/langsmith/README.md).
...
11. You can configure external clickhouse using the `values.yaml` file. You will need to provide several connection parameters for your ClickHouse instance.
12. If using TLS, make sure to set `clickhouse.external.tls` to `true`.
13. We only officially support ClickHouse versions >= 23. We also only support standalone ClickHouse or ClickHouse Cloud(not clustered or replicated).
