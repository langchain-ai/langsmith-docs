...
8. External Redis(optional).
   1. You can configure external redis using the `.env` file. You will need to provide a connection url for your redis instance.
...
- We plan to release new minor versions of the LangSmith application every 6 weeks. This will include release notes and all changes should be backwards compatible.

To upgrade, you will need to restart your LangSmith instance specifying the new version. You can do this by updating the `_LANGSMITH_IMAGE_VERSION` environment variable in the `.env` file or in the `docker-compose.yml` file.
You may also need to update your docker-compose file to match the file in the LangSmith SDK GitHub repository.
