...
LangSmith has rate and usage limits which are designed to ensure the stability of the service for all users while allowing for a continued low- or no-cost free basic service for personal development projects.
...
This 429 is the the result of exceeding a fixed number of API calls in 1 minute and is evaluated across a fixed 1 minute window on a per API key/access token basis. The start of the window will vary slightly — it is not guaranteed to start at the start of a clock minute — and may change depending on application deployment events.
...
### Plan-level hourly trace ingest limit

This 429 is the result of reaching your maximum hourly traces ingested and is evaluated in a fixed window starting at the beginning of each clock hour in UTC and resets at the top of each new hour.
...
| Plan                             | Limit          | Window |
| -------------------------------- | -------------- | ------ |
| Developer (no payment on file)   | 50,000 traces  | 1 hour |
| Developer (with payment on file) | 250,000 traces | 1 hour |
| Startup/Plus                     | 500,000 traces | 1 hour |
| Entertprise                      | Custom         | Custom |

### Plan-level hourly trace storage limit
...
This 429 is the result of reaching your maximum monthly traces ingested and is evaluated in a fixed window starting at the beginning of each calendar month in UTC and resets at the top of each new month.
...
### Self-configured monthly trace limit

Administrators may configure a trace ingest limit to prevent an unexpectedly high spend and is evaluated in a fixed window starting at the beginning of each calendar month in UTC and resets at the top of each new month.
...
If that is the case, we would like to discuss your needs more specifically as a self hosted plan or custom enterprise solution may be the right approach to ensure higher throughput.

Please reach out to [LangSmith Support](mailto:support@langchain.dev) with details about your applications throughput needs and sample code and we can work with you to better understand whether the best approach is fixing a bug, changes to your application code, or a different LangSmith plan.
:::
