...
### Cost breakdown
The Base Charge for a trace is $.0005 per trace. We priced the upgrade such that an `extended` retention trace
costs 10x the price of a base tier trace ($.005 per trace) including both metrics. Thus, each upgrade costs $.0045.
