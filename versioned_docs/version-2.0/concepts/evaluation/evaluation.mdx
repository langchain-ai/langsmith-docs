...
Once you have an application in production, you start get valuable information: how are users actually using it?
...
The most complete way to evaluate an agent is to evaluate how it does overall on a task.
This basically involves treating the agent as a black box and just evaluating whether it gets the job done or not.

The inputs should be the user input and (optionally) a list of tools.
Whether or not the list of tools is passed as input depends on whether the agent is a generic one or not.
If the agent is generic, then that means it does not have a fixed set of tools, and that needs to be passed in at run time.
If the agent is more specific, then those tools are likely hardcoded as part of the agent, and they don't need to be passed in.

The output should be the agent's final response.

The evaluators here vary depending on the task you are asking the agent to do.
Most agents do relatively complex things, and so as a result the evaluator is often (but not always) an LLM-as-judge.

This is good to do because this ultimately the thing that matters most - did it get the job done.
There are several downsides to this type of evaluation. First, it usually takes a while to run. Second, you are not evaluating anything that
happens inside the application, so it can make it tough to see where exactly it is failing. Third, it can sometimes be hard to define appropriate evaluation metrics.

### Evaluating a single step of an agent

Agents generally make multiple actions. While it is useful to evaluate them end-to-end,
it can also be useful to evaluate the individual actions.
This generally involves evaluating a single step of the agent - the LLM call where it decides what to do.

The inputs should be the input to a single step. Depending on what you are testing, this could just be the raw user input,
or it could be user input as well as previously completed steps (if you want to evaluate a downstream step).

The outputs are just the output of that step - usually the LLM response. This LLM response often contains tool calls,
indicating what action the agent should take next.

The evaluator for this is usually some binary score for whether the correct tool was used or not, as well as some heuristic for whether the input to the tool was correct.

There are several benefits to this type of evaluation.
It allows you to evaluate individual actions, which lets you hone in where your application may be failing.
They are also relatively fast to run (because they only involve a single LLM call).
One downside is that they don't capture the full agent - only one particular point.
Another downside is that while it is pretty easy to generate a dataset for steps early on in an agent's trajectory,
it can be difficult to generate a dataset for steps later on in the trajectory.
...
The outputs are a list of tool calls.

The evaluator here is some function over the steps taken. It could be a single binary score to indicate whether the "correct" trajectory was taken,
where "correct" is determined by looking at an exact match for tool name. This is simple, but has some flaws.
Sometimes there can be multiple correct paths. This evaluation also does not capture the difference between a trajectory being off by a single step versus being completely wrong.
Another evaluation metric may be number of "incorrect" steps taken. This better accounts for trajectories that are close versus ones that are way off.
Neither of these capture whether the input to the tools is correct.
In order to account for that, another evaluation technique is to pass the agent's trajectory (along with a desired trajectory) to an LLM and ask if it is correct.

The benefits of this type of evaluation are that you can evaluate a full run of the agent while also evaluating what went on under the hood.
One downside is that you have to have a good record of what those steps taken were (luckily, using a framework like LangGraph can help with that!).
Another downside is that evaluation metrics can be somewhat tricky to come up with.
