...
The pace of AI applications development is often rate-limited by high-quality evaluations. In LLM application development today, there is a paradox of choice. LLM prompts are arbitrary text and strongly influence LLM behavior, create a wide range of possible ways to prompt your application. There's a growing number of LLMs to choose from, making it difficult to find one that best balances accuracy, latency, and cost for your application. The paradox of choice can slow down and confuse LLM application development. High quality evaluations can help you rapidly answer these types of questions with confidence.

LangSmith allows you to build high-quality evaluations for your AI application. This conceptual guide will give you the foundations to get started.

The core components of LangSmith evaluation include:
...
In the sections below, we’ll explore the fundamentals of `Datasets` and `Evaluators`, discuss how to evaluate various types of LLM applications, and provide a decision tree to help you select the right evaluation approach for your application.

![Summary](../static/langsmith_summary.png)

## Datasets

Datasets are the cornerstone of the LangSmith evaluation workflow. They are collections of examples that provide the necessary inputs and, optionally, expected `reference` outputs for assessing your AI application.

Each example within a dataset represents a single data point, consisting of an `inputs` dictionary, an optional `output` dictionary, and an optional `metadata` dictionary. The `optional` output dictionary will often contain a `reference` key, which is the expected LLM application output for the given input.
...
LangSmith offers three distinct dataset types — `kv`, `llm`, and `chat`— designed to accommodate specific input and output schemas.
...
Evaluators are functions in LangSmith that score how well your Task (application) performs on a particular example, serving as a judge.

During an evaluation, your examples from your dataset are processed through your task. The inputs to an evaluator include:
...
To go deeper on this theme, see our [video](https://www.youtube.com/watch?v=Vn8A3BxfplE) that shows how to use LangSmith to add human feedback to LLM outputs and use this for prompt engineering.
...
2. `Heuristic`:
   ...
   For some tasks, like code generation, custom heuristic evaluation (e.g., import and code execution-evaluation) are often extremely useful and superior to LLM-as-judge (discussed below).
...
We can visualize the above ideas collectively in this diagram. To review, `datasets` are composed of `examples` can be curated from a variety of sources such as historical logs or user curated examples. `Evaluators` are functions that score how well your `task` (application) performs on each `example` in your `dataset`. Evaluators can use different scoring functions, such as `human`, `heuristic`, `LLM-as-judge`, or `pairwise`. And if the `dataset` contains `reference` outputs, then the evaluator can compare the `task` output to the `reference` output.
...
### Building Confidence: Repetitions

One of the most common questions when evaluating AI applications is how to build confidence the result of an experiment? This is particularly relevant for LLM applications (e.g., agents), which can exhibit considerable run-to-run variability.

Repetitions involve running the same evaluation multiple times and aggregating the results to smooth out run-to-run variability and examine the reproducibility of the AI application's performance. LangSmith allows you to easily set the number of repetitions and aggregates the results for you in the UI.
...
1. Unit Tests:
   ...
   - Unit tests are a familiar to most developers, often used in software development to verify the correctness of individual system components.
   - In the context of LLM applications, [unit tests are often lightweight assertions](https://hamel.dev/blog/posts/evals/#level-1-unit-tests) on LLM inputs or outputs (e.g., type or schema checks).
   - Often these are triggered by any change to your application as quick assertions of basic functionality.
...
2. Regression Testing:
   ...
   - Regression testing is often used to measure performance across versions of your application over time.
   - Typically, they are used to ensure a new version of your application does not regress on any examples that your current version is passing.
   - In practice, they help you assess how much better or worse your new version is relative to the baseline.
   - Often these are triggered when you are making an update to your application that is expected to influence the user experience.
...
LangSmith's comparison view has native support for regression testing, allowing your to quickly see examples that have changed relative to the baseline (highlighted in red or green):

![Regression](../static/regression.png)

3. Back-testing:

   - Back-testing is an approach that combines dataset creation (discussed above) with evaluation.
   - If you have a collection of production logs, you can turn them into a dataset.
   - Then, you can re-run those production examples with with newer application versions.
   - This allows you to assess performance on past and realistic user inputs.
...
- `Response`: Evaluate the agent response against an `reference` response. This does not evaluate any intermediate tool calls, but rather focuses only on the final agent response.
- `Single node`: Evaluate any agent node in isolation. For example, this can focus specifically on the `assistant node` and evaluate whether it selects the appropriate tool given an input prompt.
...
- Use repetitions to smooth out noise, as tool selection and agent behavior can show run-to-run variability. See the [video on `Repetitions` in our LangSmith Evaluation series](https://youtu.be/Pvz24JdzzF8)
