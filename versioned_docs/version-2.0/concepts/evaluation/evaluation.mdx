...
We frequently hear users ask how to get started with evaluation.

### Collecting evaluation data

![Flywheel](../static/flywheel.png)

Consider your evaluation process to be a flywheel, a virtuous cycle that results in improved application performance over time. First, you have an application in production without any evaluation. You are collecting logs from user interactions with your app, captured in LangSmith. First, consider adding `online evaluation` to your app. This can be as simple as guardrails checking for toxicity in user inputs or outputs. In LangSmith, you can quickly filter for traces that fail online evaluation, which can be examined more deeply. Second, consider adding production logs, optionally filtering on cases where `online evaluation` fails, to a `Dataset` for `Back-testing`.
...
`Back-testing` newer app versions on production logs can be a powerful way to assess how new versions of your app perform on historical user interactions. With some confidence that we've improved our app, we can then consider adding hard or interesting cases from our production logs to a fixed, `manually curated` a dataset. At this stage, we might also consider adding a `reference` output to the examples in our fixed dataset.

This fixed dataset can be used for `regression testing`, as it contains a growing set of curated, hard examples we've encountered in production. The aim here is to ensure that newer app version do not regress on past cases that we've fixed! This seems obvious, but over time it can be common for a new app version to improve on recent hard cases but regress on older hard cases that we've long forogtten about. Regression testing ensures that you app performance is improving continuously over time. Finally, `unit tests` can be added to your app to ensure that basic functionality is working as expected prior to deployment.

### Choose your scoring function

![Score](../static/score_fxn.png)

Scoring functions are applied in all of the evaluation approaches we've discussed in the flywheel above and should be carefully designed for each one. In general, a well designed scoring function should have a low cognitive burden, resulting in higher reliability. For example, scoring the factual correctness of a RAG response [on a scale](https://en.wikipedia.org/wiki/Likert_scale) of 1 (worst) to 10 (best) is cognitively demanding and, in our experience, unreliable. A human or LLM-as-judge evaluator is less likely to grade consistently with such an open-ended grading scale.
...
But, if you are confident in your ability to assign individual scores to our application output, then ask whether your scoring function can be a hard-coded rule. Consider the simplest case: you want to check to see if the output is empty. This can be easily hard-coded in a custom heuristic evaluator, and is commonly employed in unit tests.
