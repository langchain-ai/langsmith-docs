...
See our in depth tutorial on [evaluating agents](https://docs.smith.langchain.com/tutorials/Developers/agents).
...
The outputs are a list of tool calls, which can be formulated as an "extact" trajectory (e.g., an expected sequence of tool calls) or simply a list of tool calls that are expected (in any order).
...
To address these flaws, evaluation metrics can focused on the number of "incorrect" steps taken, which better accounts for trajectories that are close versus ones that deviate significatly. Evaluation metrics can also focus on whether all of the expected tools are called in any order.

However, none of these approaches evaluate the input to the tools; they only focus on the tools selected. In order to account for this, another evaluation technique is to pass the full agent's trajectory (along with a reference trajectory) as a set of messages (e.g., all LLM responses and tool calls) an LLM-as-judge. This can evaluate the complete behavior of agent, but it the most challenging reference to compile (luckily, using a framework like LangGraph can help with this!). Another downside is that evaluation metrics can be somewhat tricky to come up with.
