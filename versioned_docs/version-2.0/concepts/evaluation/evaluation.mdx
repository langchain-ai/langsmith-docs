...
4. `Pairwise`:
   ...
   It [can be easier](https://www.oreilly.com/radar/what-we-learned-from-a-year-of-building-with-llms-part-i/) for a human (or an LLM grader) to determine `A is better than B` than to assign an individual score to either A or B, which explains why pairwise evaluations can be more stable than assigning individual scores.
...
1. `Unit Tests`:
   ...
   - Unit tests are often used in software development to verify the correctness of individual system components.
   - [Unit tests are often lightweight assertions](https://hamel.dev/blog/posts/evals/#level-1-unit-tests) on LLM inputs or outputs (e.g., type or schema checks).
...
2. `Regression Testing`:
   ...
   - They are used to ensure new app versions do not regress on examples that your current version is passing.
   - In practice, they help you assess how much better or worse your new version is relative to the baseline.
   - Often these are triggered when you are making app updates that are expected to influence the user experience.
