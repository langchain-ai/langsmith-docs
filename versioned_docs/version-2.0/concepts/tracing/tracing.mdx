...
This conceptual guice covers topics that are important to understand when logging traces to LangSmith. A `Trace` is essentially a series of steps that your application takes to go from input to output. Each of these individual steps is represented by a `Run`. A `Project` is simply a collection of traces. The following diagram displays these concepts in the context of a simple RAG app, which retrieves documents from an index and generates an answer.
...
1. [In the UI] - Annotate runs directly in the UI
2. [With the SDK] - Programmatically log feedback to LangSmith
3. [With Online Evaluators] - Automatically run evaluators on a sample of your production traces to generate feedback in realtime
