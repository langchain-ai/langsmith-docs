...
const generateWikiSearch = traceable(async (input: { question: string }) => {
  const messages = [
    {
...
const retrieve = traceable(async (input: { query: string; numDocuments: number }) => {
  ...
  return finalResults;
}, { name: "generateWikiSearch" , run_type: "retriever" });\n
const generateAnswer = traceable(async (input: { question: string; context: string }) => {
  const messages = [
    {
      role: "system" as const,
...
const ragPipeline = traceable(async ({ question }, numDocuments: number = 2) => {
...
The key here is that the evaluator function should traverse the `root_run` argument to access the intermediate steps of the pipeline. The evaluator can then process the inputs and outputs of the intermediate steps to evaluate according to the desired criteria.
