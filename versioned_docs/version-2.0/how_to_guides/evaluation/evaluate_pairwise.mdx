...
LangSmith supports evaluating existing experiments against each other. This allows you to use an LLM to judge outputs from multiple systems against each other, rather than being confined to evaluating outputs one at a time. Think [Chatbot Arena](https://chat.lmsys.org/) - this is the same concept! To do this, use the `evaluate_comparative` function
...
```
This example simply assigns the better score to each of the runs in `test-mundane-strait-65`.
