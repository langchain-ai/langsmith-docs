...
<CodeTabs
  tabs={[
    ...
    `,
    typescript`
      import type { StructuredPrompt } from "@langchain/core/prompts";
      ...
      const prompt: StructuredPrompt = await hub.pull(
        "langchain-ai/pairwise-evaluation-2"
      );
      ...
      async function evaluatePairwise(runs: Run[], example: Example) {
        ...
        const response = await runnable.invoke(
          ...
          // This is required to properly handoff tracing
          // from LangSmith to LangChain
          { callbacks: await getLangChainCallbacks() }
        );
        
        const score = response?.["Preference"];
      ...
      await evaluateComparative(["my-experiment-name-1", "my-experiment-name-2"], {
