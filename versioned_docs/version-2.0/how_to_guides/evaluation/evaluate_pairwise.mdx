...
:::note
Pairwise evaluation currently requires `langsmith` Python version `>=0.1.55` or JS version `>=0.1.24`.
:::

At its simplest, `evaluate_comparative` / `evaluateComparitive` function takes the following arguments:
...
- `randomize_order` / `randomizeOrder`: An optional boolean indicating whether the order of the outputs should be randomized for each evaluation. This is a strategy for minimizing positional bias in your prompt: often, the LLM will be biased
