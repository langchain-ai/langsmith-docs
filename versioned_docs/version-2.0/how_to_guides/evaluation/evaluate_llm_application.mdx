...
# First, create an example with metadata
...
# Then, later, you can fetch examples that match the metadata filter
results = evaluate(
    lambda inputs: label_text(inputs["text"]),
    data=client.list_examples(dataset_name=dataset_name, metadata='{"foo": "bar"}'),
    evaluators=[correct_label],
