...
- [How-to guide on managing datasets](../datasets/managee_datasets_in_application)
...
The following example involves evaluating a very simple LLM pipeline as classifier to label input data as "toxic" or "non-toxic".
...
The `root_run` is always available and contains the inputs and outputs of the target task. If tracing is enabled, the `root_run` will also contain child runs for each step in the pipeline.
...
To view a more advanced example that traverses the `root_run` object, please refer to [this guide](./evaluate_on_intermediate_steps) on evaluating on intermediate steps.

## Use off-the-shelf LangChain evaluators

LangSmith provides a number of off-the-shelf evaluators that can be used to evaluate the performance of your LLM application.
...
Simply use `list_examples` to fetch examples from a particular version tag using `as_of`.
...
import { CodeTabs } from "../../../../src/components/InstructionsWithCode"
