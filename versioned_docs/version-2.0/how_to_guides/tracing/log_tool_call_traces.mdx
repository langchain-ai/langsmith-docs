...
If you want to log tool calls but your tools are function names in a schema dictionary or JSON, then manually trace it with `langsmith.trace` at the point where your tool is executed or chosen.
...
The following code snippets show how to log a schema-based tool call in Python. You'd add `langsmith.trace` right when the tool is chosen / executed.

Here's an example of a schema-based tool definition, and logging the tool trace when the tool is executed.
