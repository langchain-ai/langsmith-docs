...
@traceable
def format_prompt(subject):  # highlight-next-line
...
def invoke_llm(messages):  # highlight-next-line
    run = get_current_run_tree()
...
@traceable
def parse_output(response):  # highlight-next-line
    run = get_current_run_tree()
...
@traceable
def run_pipeline():  # highlight-next-line
    run = get_current_run_tree()
