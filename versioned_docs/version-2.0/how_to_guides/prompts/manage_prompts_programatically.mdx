...
print(url)
`),
    LangChainPyBlock(`from langchain import hub
from langchain_core.prompts import ChatPromptTemplate\n
prompt = ChatPromptTemplate.from_template("tell me a joke about {topic}")
url = hub.push("joke-generator", prompt)
# url is a link to the prompt in the UI
print(url)
`),
...
url = prompts.push("joke-generator", chain);
// url is a link to the prompt in the UI
console.log(url);
`),
  ]}
...
prompt = ChatPromptTemplate.from_template("tell me a joke about {topic}")
chain = prompt | model\n
client.push_prompt("joke-generator-with-model", chain)
`),
...
prompts.push("joke-generator-with-model", chain);`),
  ]}
  groupId="client-language"
/>
...
chain.invoke({"topic": "cats"})`),
    LangChainPyBlock(`from langchain import hub as prompts
chain = prompts.pull("joke-generator", prompt)
# url is a link to the prompt in the UI
print(url)
`),
    TypeScriptBlock(`import * as prompts from "langchain/hub";
...
// List my private prompts that include "joke"
const prompts = client.listPrompts({ query: "joke", isPublic: false});
// Delete a prompt
