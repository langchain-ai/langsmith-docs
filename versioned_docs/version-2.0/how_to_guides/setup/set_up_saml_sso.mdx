...
1. Enter the following information in the `Conofigure SAML` page:
   1. `Single sign-on URL` a.k.a. `ACS URL`: <RegionalUrl suffix='/auth/v1/sso/saml/acs'/>. Keep `Use this for Recipient URL and Destination URL` checked.
   1. `Audience URI (SP Entity ID)`: <RegionalUrl suffix='/auth/v1/sso/saml/metadata'/>
