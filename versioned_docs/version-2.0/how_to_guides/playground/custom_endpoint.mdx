...
You will need to implement and deploy a model server to utilize this feature. The model server must be accessible from the LangSmith Playground service.
:::
...
Once you have deployed a model server, you can use it in the LangSmith Playground. Enter the playground and select either the `ChatCustomModel` or the `CustomModel` provider for chat-style model or instruct-type models.
