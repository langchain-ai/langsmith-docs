...
To view run rule logs applied to older runs than 1 day, you can select **Last 1 day** and enter the desired date range. As backfills will start from the start date and go forward, this may be needed to view logs while the backfill is proceeding.
