...
LangSmith has LLM-native observability, letting you get all sorts of insights into your application.
In this tutorial we will walk through an example of setting up observability for your application.
...
We will go about adding observability into this application.
The simple application we're adding observability into looks something like:
...
Having observability set up from the start can you help iterate **much** more quickly that normal.
It allows you to have great visibility into your application as you are rapidly changing the prompt, model, data, etc.
...
First, create an API key by navigating to the <a href="https://smith.langchain.com/settings">settings page</a>.
...
Finally, set up the appropriate environment variables. This will log traces to `default` project (though you can easily change that).
...
LangSmith is **built** for tracing the whole trace - so let's do that!
...
This will produce a trace of just the whole chain (with the OpenAI call as a sub-run) - it should look something like [this](https://smith.langchain.com/public/2174f4e9-48ab-4f9e-a8c4-470372d976f1/r)
...
This will produce a trace of just the whole chain INCLUDING the retrieval step - it should look something like [this](https://smith.langchain.com/public/7b3fe3b1-b287-49ec-9e3b-feecbe460144/r)
...
Notice we added `@traceable(metadata={"llm": "gpt-3.5-turbo"})` to the `rag` function.
...
You can also use this tab to perform a version of AB Testing.
...
If you have more in-depth questions about observability, check out the how-to section for guides.

Observability is not the only thing LangSmith can help with!
It can also help with testing, prompt management, and more!
