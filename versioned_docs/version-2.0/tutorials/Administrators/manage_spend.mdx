...
This tenant is currently using extended data retention tracing, which explains why both metrics are roughly equal each day.
...
case that this tenant is using extended data retention tracing everywhere. As such, we start by optimizing our retention settings.
...
If we are not a big enterprise, we may shutter at the ~$40k per month bill at Plus plan prices.
