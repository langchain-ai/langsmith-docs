...
# V2: Optimize tracing spend on LangSmith
...
LangSmith Traces usage is measured per workspace. This is because workspaces often represent development environments (as in our example),
or teams within an organization, and as a LangSmith administrator, we want to know which environment or team is responsible for
...
go to `Settings` -> `Workspaces`, and hover our mouse over the `Workspace ID` button to find the one with a matching ID:

![](./static/p1findworkspaceid_v2.png)

### Invoices

We understand what usage looks like in terms of traces, but need to now translate that into spend. To do so,
...
In the above GIF, we see that the charges for LangSmith Traces are broken up by "tenant_id", meaning we can track tracing spend
...
Let's say we follow the same logic for our dev and staging environments, which leads us to set limits at 10% of the production
