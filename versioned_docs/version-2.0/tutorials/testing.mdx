# Test & Evaluate your Application

It can be hard to measure the performance of your application.
Yet having some way to do so is crucial.
...
- Run evaluations on a few different prompts/LLMs
...
There are a few aspects to consider here: (1) what should the schema of each datapoint be? (2) how many datapoints should I gather? (3) how should I gather those datapoints?
...
Often times you cannot define the perfect output - that's okay!
Sometimes you may also want to define more information for each datapoint - like the expected documents to fetch in RAG, or the expected steps to take as an agent.
LangSmith datasets are **super** flexible and allow you to define arbitrary schemas.
...
For most teams that are starting new project, we generally see them start by collecting the first 10-20 datapoints by hand.
...
This makes our evaluation a little bit trickier.

In addition to evaluating correctness, let's also make sure our answers are short and concise.
This will be a little bit easier - we can define a simple Python to measure the length of the response.
...
If we go back to the dataset page, we can now see a summary of our one run!
...
Awesome, we've evaluated three different runs. But how do we can compare results?
...
There are many different types of evaluators you may wish to explore.
...
For information on this, check out the other tutorials.
