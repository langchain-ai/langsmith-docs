...
Get an API key for your **personal tenant** if you have not yet. The hub will not work with your non-personal organization's api key!
...
// pull a chat prompt
const prompt = await hub.pull("efriis/my-first-repo");
...
## 4. Push it back to your own tenant as a private prompt
...
await hub.push("<handle>/my-first-repo", prompt);
```
