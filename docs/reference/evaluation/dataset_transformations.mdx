...
standardized across model providers for usage in evaluations / few shot prompting / etc. To simplify setup of transformations,
LangSmith offers a pre-defined schema that will do the following:

- Extracts messages passed into your collected runs and transform them into openai standard format, which makes them compatible all LangChain ChatModels and most model providers downstream
- Extracts any tools used by your LLM and add them to your input to be used for reproducability in downstream evaluation
