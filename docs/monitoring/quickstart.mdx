...
One common workflow is to send datapoints to a dataset automatically. This can be done by selecting the dataset button and then selecting the dataset you want to send to (or create a new dataset). If you enable "Use corrections" then, for each matched run we will loook for feedback containing a "correction" and save that as the outputs of the new dataset example. If no feedback is found, or if you turn this off, we will use the run outputs instead.
