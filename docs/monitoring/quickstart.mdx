...
If you enable "Use corrections" then, for each matched run we will loook for feedback containing a "correction" and save that as the outputs of the new dataset example. If no feedback is found, or if you turn this off, we will use the run outputs instead.
