...
When scaling up the write path (trace ingestion), it is helpful to monitor the four services/resources listed above. Here are some typical changes that can help increase perfromance of trace ingestion:
...
Below we provide some example LangSmith configurations based on expected read and write loads. Similar to the above sections, writes refer to trace ingestion, and reads refer to trace querying.

For read load:

- Low means up to 5 users looking at traces at a time (roughly 10 requests per second)
- Medium means up to 20 users looking at traces at a time (roughly 40 requests per second)
- High means up to 50 users looking at traces at a time (roughly 100 requests per second)
...
This means that you have a very high scale of trace ingestions, but not too many users on the frontend querying traces.
...
backend:
  deployment:
    replicas: 15 # OR enable autoscaling to this level (example below)
# autoscaling:
#   enabled: true
#   maxReplicas: 15
#   minReplicas: 6
...
This means you are both ingesting traces at a very high rate (approaching 1000 traces submitted per second) and also have many users querying traces on the frontend (over 50 users) and/or scripts that are consistenlty making requests to `/runs/query` or `/runs/<run-id>` endpoints.
