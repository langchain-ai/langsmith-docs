...
- We support both standalone ClickHouse and externally managed clustered deployments. For clustered deployments, ensure all nodes are running the same version. Note that clustered setups are not supported with LangSmith-managed ClickHouse installations.
...
- Clustering is only supported with externally managed ClickHouse deployments. It is not compatible with LangSmith-managed ClickHouse installations as these do not include required ZooKeeper configurations.
