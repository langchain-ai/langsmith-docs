...
1. A working Kubernetes cluster that you can access via `kubectl`. Your cluster should have the following minimum requirements:
   ...
   1. Valid Dynamic PV provisioner or PVs available on your cluster.
      ...
      - Our base installation will try to install these services inside your cluster. To enable persistence, we will try to provision volumes for these services.
...
1. Api Key Salt
   ...
   ```
1. JWT Secret (Optional but used for basic auth)
   1. This is a secret key that you can generate. It should be a random string of characters.
   1. You can generate this using the following command:
   ```bash
   openssl rand -base64 32
...
1.  Ensure you have the Langchain Helm repo added. (skip this step if you are using local charts)
...
1.  Find the latest version of the chart. You can find the available versions in the [Helm Chart repository](https://github.com/langchain-ai/helm/releases).
...
1.  Run `helm install langsmith langchain/langsmith --values langsmith_config.yaml --version <version> -n <namespace> --debug`
...
Review our [configuration section](../configuration) for more information on how to configure these options.
