...
1. Use the file from here (ADD LINK) for your config, and modify the values as you see fit.
2. Find the latest version of the chart by running `helm search repo langchain/langsmith-observability --versions`.
3. Grab the latest version number, and run `helm install langsmith-observability langchain/langsmith-observability --values langsmith_obs_config.yaml --version <version> -n namespace --debug`
...
This section will show you how to deploy the end-to-end observability stack for LangSmith, using the Helm chart (LINK HERE).
...
helm repo update'
helm install opentelemetry-operator open-telemetry/opentelemetry-operator -n langsmith-observability
```

## Installation

### Option 1: LGTM Stack + MinIO
This option brings up the OTel collectors, LGTM stack and uses [MinIO](https://github.com/minio/minio) for object storage, which is an in-cluster S3-compatible object storage.
:::warning Important
...
In order to install this, use the configuration from the `e2e-stack-minio.yaml` file. INSERT LINK.
...
### Option 2: LGTM Stack + Object Storage
You should always use this option for production environments. It will store your data into external object storage.
The LGTM stack supports Amazon S3, Google Cloud Storage, Azure Blob Storage and OpenStack Object Storage.

And example of required values to deploy with S3 can be found in the `e2e-stack-s3.yaml` file (INSERT LINK HERE). It is up to you whether to use a shared bucket for all 3 services, or separate ones.
...
1. Loki [Here](https://grafana.com/docs/loki/latest/configure/storage/)
2. Mimir [Here](https://grafana.com/docs/mimir/latest/configure/configure-object-storage-backend/)
3. Tempo [Here](https://grafana.com/docs/tempo/latest/configuration/hosted-storage/)
:::
...
Once applied, you should see each pod in your LangSmith deployment restart with a sidecar collector.
...
Use the username `admin` and the password from your secret to log into Grafana.
