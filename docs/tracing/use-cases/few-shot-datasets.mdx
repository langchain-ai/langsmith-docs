...
```bash
pip install -U langchain openai langsmith gpt4all
```
...
prototype_results = await arun_on_dataset(
...
for q in dev_questions:
    client.create_example(inputs={"input": q}, dataset_id=dev_dataset.id)
```
...
# Pass in a constructor function to run the chain
def construct_dev_chain():
    return LLMChain.from_string(
        gpt4all_model,
        template="Help out as best you can.\nQuestion: {input}\nResponse: ",
    )
...
original_dev_res = run_on_dataset(
    dataset_name=dev_dataset_name,
    llm_or_chain_factory=construct_dev_chain,
...
```python
def construct_few_shot_chain():
    return LLMChain(
        llm=gpt4all_model,
        prompt=few_shot_prompt,
    )

few_shot_dev_res = run_on_dataset(
    dataset_name=dev_dataset_name,
    llm_or_chain_factory=construct_few_shot_chain,
    evaluation=RunEvalConfig(
