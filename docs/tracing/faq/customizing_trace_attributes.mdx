...
You can also customize and override this behavior for a given Client instance.
...
def filter_outputs(outputs: dict):
    ...
    return {}\n
llm = ChatOpenAI()\n
with tracing_v2_enabled("test-filtering", client=Client(hide_inputs=filter_inputs, hide_outputs=filter_outputs)) as cb:
    llm.invoke("Say foo")
...
const tracer = new LangChainTracer({ hideInputs: (inputs) => ({}), hideOutputs: (outputs) => ({}) });
...
langsmith_client = Client(hide_inputs=lambda inputs: {}, hide_outputs=lambda outputs: {})\n
...
const langsmithClient = new Client({ hideInputs: (inputs) => ({}), hideOutputs: (outputs) => ({}) });\n
// The linked run will have its metadata present, but the inputs will be hidden
...
langsmithExtra: { client: langsmithClient }
});
// The linked run will not have hidden inputs and outputs
await openaiClient.chat.completions.create({
model: "gpt-3.5-turbo",
