...
Sometimes your application may span multiple services. LangSmith supports exporting context propagation headers to maintain a trace across processes and systems.
The following example shows an example 'client' and 'server' setup, where the trace starts on the client and continues on the server.
