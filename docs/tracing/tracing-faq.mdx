...
Each run contains a unique ID, name, inputs, and optionally outputs, error information, tags, and other metadata.

A 'Trace' is a collection of runs organized in a tree or graph. The 'Root Run' is the top level run in a trace, which is assigned an execution_order=1. It is the run that is directly invoked by the user or application.

The execution order of a run is the order in which it was called in the trace.
