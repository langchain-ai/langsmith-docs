...
use the LangSmith client. Below is an example.
...
There may be a period of time between when you first post a run and when it is retrievable via the API. You may need to wait a few seconds before the run is available.
