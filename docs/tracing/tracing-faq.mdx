...
### How do I organize traces within a project?
...
chain.invoke({"input": "Hello, World!"}, {"tags": ["shared-tags"]})`),
    TypeScriptBlock(`import { PromptTemplate } from "langchain/prompts";
...
With chat bots, copilots, and other common LLM design patterns, users frequently interact with your model over multiple interactions. Each invocation of your model is logged as a separate trace, but you can group these traces together using metadata (see [how to add metadata to a run](#how-do-i-add-metadata-to-runs) above for more information). Below is a minimal example:

<CodeTabs
  tabs={[
    PythonBlock(`from langchain_core.messages import AIMessage, HumanMessage
...
![Conversation Traces](./static/conversation_search.png)
...
You can customize chain and runnable trace names by calling `new_chain = chain.with_config({"run_name": "MyRunName"})`. (Note: this is not currently supported directly on LLM objects.)

Example:

<CodeTabs
  tabs={[
    PythonBlock(`configured_chain = chain.with_config({"run_name": "MyCustomChain"})
configured_chain.invoke({"query": "What is the meaning of life?"})`),
    TypeScriptBlock(`const configuredChain = chain.withConfig({ runName: "MyCustomChain" });
...
### How do I query by metadata?

To query a run or trace by metadata, you can use the `has` filter. This filter takes a key and value and returns all runs that have a metadata key with the specified value. Below is an example:
...
runs = list(client.list_runs(
    ...
    filter='has(metadata, \\'{"user_id": "abc123"}\\')',
))`),
...
For more information on fetching runs from the API, check out the [local run filtering](use-cases/export-runs/local) guide.
...
See the [trace](https://smith.langchain.com/public/e13ce247-bda1-44b5-bdb9-76443e296cbb/r) for a visualization of the run tree generated by this code.
...
<CodeTabs
  tabs={[
    PythonBlock(`from langsmith import Client\n
client = Client()
client.delete_project(project_name="<your_project>")`),
    TypeScriptBlock(`import { Client } from "langsmith";\n
const client = new Client();
await client.deleteProject({projectName: "<your_project>"});
`),
  ]}
  groupId="client-language"
/>
...
### How do I reduce my trace count?
