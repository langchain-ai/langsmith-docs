...
LangSmith can now accept traces from from OpenTelemetry based clients.
...
```bash
export OTEL_OTEL_EXPORTER_OTLP_HEADERS=https://api.smith.langchain.com/otel
export OTEL_EXPORTER_OTLP_HEADERS="x-api-key=<your langsmith api key>"
```

If you'd like to log to a different project than default, you can add the LANGSMITH_PROJECT:
...
def call_openai():
    ...
    with tracer.start_as_current_span("call_open_ai") as span:
        ...
        span.set_attribute("gen_ai.completion.0.role", "asistant")
...
You can also specify a project name if you'd like:

```bash
TRACELOOP_HEADERS=x-api-key=<your_langsmith_api_key>,LANGMITH_PROJECT=<langsmith_project_name>
```
