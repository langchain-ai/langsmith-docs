...
LangSmith supports uploading binary files (such as images, audio, videos, and PDFs) with your traces. This is particularly useful when working with LLM models pipelines using multimodal inputs or outputs.
...
@traceable
def trace_with_attachments(val: int, text: str, image: Attachment, audio: Attachment, video: Attachment, pdf: Attachment):
...
In the TypeScript SDK, the \`extractAttachments\` function is an optional parameter in the \`traceable\` configuration that allows you to define which parts of your function’s input should be treated as attachments, specifying their MIME types and data. This function is called whenever you invoke the traceable-wrapped function, and it enables you to extract binary data (e.g., images, audio files) from your inputs and log them alongside other trace data.
