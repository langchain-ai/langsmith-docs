...
1. **Decorator Order**: The order of decorators matters. Place the LangSmith `@traceable` decorator before the PromptFlow `@tool` decorator.

2. **Return Type**: Make sure to correctly specify the return type of your function in the function signature to match what you're actually returning.

3. **Headers Propagation**: Always check if the run_tree exists before calling to_headers() to avoid errors.
