...
<CodeTabs
  tabs={[
    PythonBlock(`# You can opt-in to specific invocations..
...
<CodeTabs
  tabs={[
    PythonBlock(`# You can set the project name using the project_name parameter.
import langsmith as ls\n
...
<CodeTabs
  tabs={[
    PythonBlock(`import langsmith as ls\n
...
You can trace multimodal chains that involve images in LangSmith. There are two main ways to include images in your prompts:

* Using base64-encoded images
* Using direct URLs

:::tip
If you're not using LangChain's `ChatPromptTemplate`, check out [this guide](./log_multimodal_traces) for other ways to log multimodal traces. For additional types of multimodal inputs and examples, see the [LangChain multimodal inputs guide](https://python.langchain.com/docs/how_to/multimodal_inputs/).
:::
...
async function createImageMessages(imagesInfo: ImageInfo[]): Promise<Array<[string, any[]]>> {
  ...
  for (const imgInfo of imagesInfo) {
    ...
    const imageData = pathOrUrl.startsWith('http')
      ...
      : {
          type: "image_url",
          image_url: {
            url: \`data:image/jpeg;base64,\${await encodeImageToBase64(pathOrUrl)}\`
          }
        };
...
const imageMessages = await createImageMessages(imagesInfo);
