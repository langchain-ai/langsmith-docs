...
Run metadata and tags are also powerful to filter on. These relies on good tagging for all parts of your pipeline. To learn more, you can check out [this guide](./add_metadata_tags).
...
If you would like to filter for all runs _whose tree contains_ the tool filter call, you can use the tree filter in the [advanced filters](/observability/how_to_guides/filter_traces_in_application#advanced-filters) setting:
