...
## Example on filtering using output key-value pairs: filter tool calls

As an example of filtering on key-value pairs, let's suppose that you wanted to filter traces for tool calls.

In this case, let's assuming this is your output you want to filter for:
...
Our system will break it into a set of key value matches:
...
To search for tool calls, you can use the following search:

`generations.message.kwargs.tool_calls.type` = `tool_call`

![Filtering](./static/search_kv_tool.png)

This will match runs where the `tool_calls` type is `tool_call`.
