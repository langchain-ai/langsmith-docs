...
Our system will break it into a set of searchable key value matches:
...
To search for specific tool calls, you can use the following search:

`generations.message.kwargs.tool_calls.name` = `Plan`

![Filtering](./static/search_kv_tool.png)

This will match root and non-roto runs where the `tool_calls` type is `tool_call`.

If you would like to filter for runs whose tree contains this tool filter call, you can set this filter in the advanced filters setting:
