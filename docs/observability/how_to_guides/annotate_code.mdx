...
LangSmith's tracing is done in a background thread to avoid obstructing your production application. This means that your process may end before all traces are successfully posted to LangSmith.
Here are some options for ensuring all traces are submitted before exiting your application.
...
async def my_traced_func(): # Your code here...
pass
...
If you are using LangChain, please refer to our [LangChain tracing guide](./trace_with_langchain#ensure-all-traces-are-submitted-before-exiting).
