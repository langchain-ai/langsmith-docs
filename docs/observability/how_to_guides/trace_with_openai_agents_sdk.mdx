...
You can integrate LangSmith tracing with the OpenAI Agents SDK by using the `LangsmithTracingProcessor` class.

```python
import asyncio
from agents import Agent, Runner, set_trace_processors
from langsmith.wrappers import LangsmithTracingProcessor
...
if __name__ == "__main__":
    set_trace_processors([LangsmithTracingProcessor()])
    asyncio.run(main())
```
