...
You can also provide the following `metadata` fields to help LangSmith identify the model and calculate costs. If using LangChain or the LangSmith [OpenAI wrapper](./annotate_code#wrap-the-openai-client), these fields will automatically be populated correctly. To learn more about how to use the `metadata` fields, see [this guide](./add_metadata_tags).
