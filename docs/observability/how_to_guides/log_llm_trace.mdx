...
calculates costs automatically by using the <RegionalUrl text='model pricing map' suffix='/settings/workspaces/models' />.

:::tip LangSmith token/cost tracking
To learn how LangSmith calculates token and cost statistics automatically, see [this guide](./calculate_token_based_costs).
:::

However, many models already include exact token counts and/or cost information as part of the response. If you have this information, you can override the default token/cost calculation in LangSmith in one of two ways:
...
In both cases, the usage metadata you send should contain a subset of the following LangSmith-recognized fields (additional fields are not allowed):
