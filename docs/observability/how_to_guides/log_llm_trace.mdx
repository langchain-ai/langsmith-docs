...
You may also trace your model calls using [`traceable`](../annotate_code/). To properly mark the traced function
as an LLM run, at bare minimum you must pass `run_type` as `"llm"` like this:
