...
## Explicitly setting costs while tracing
...
dict while tracing rather than relying on LangSmith's built-in token counting.

See [this section](./log_llm_trace#manually-provide-usage-information) on manually providing usage information for an example.
