...
def test_sql_generation_select_all(user_query, expected_sql):
    sql = generate_sql(user_query)
...
def test_sql_generation(user_query, expected_sql):
    sql = generate_sql(user_query)
    assert sql == expected_sql
```
...
:::note
`@unit` is currently only supported in python. If you are interested in unit testing functionality in TypeScript or other languages, please let us know at [support@langchain.dev](mailto:support@langchain.dev).
:::