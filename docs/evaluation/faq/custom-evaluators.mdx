    ...
    tools = [
        {
            ...
            "function": {
                "name": "propositions",
                "description": "Use to score how faithful the .",
...
def multiple_scores(run: Run, example: Example) -> dict:
    ...
    # Formulating the input prompt for the OpenAI chat completion
    messages = [
        {
            "role": "user",
    ...
    # Configuring the tool for structured scoring output
    tools = [
        {
            "type": "function",
            "function": {
                "name": "scores",  # Adjust according to the actual function name if different
