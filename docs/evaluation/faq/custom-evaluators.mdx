...
scorer = ChatOpenAI(model="gpt-4-turbo-preview").with_structured_output(Scores)

def multiple_scores(run: Run, example: Example) -> dict:
    reference = example.outputs["answer"]
    prediction = run.outputs["output"]

    scores = scorer.invoke(f"Reference: {reference}\nPrediction: {prediction}")

    return {
        "results": [
            {"key": "correctness", "score": scores.correctness, "comment": scores.correctness_reasoning},
            {"key": "conciseness", "score": scores.conciseness, "comment": scores.conciseness_reasoning}
        ]
    }
```
