...
<CodeTabs
  tabs={[
      python`
        from langchain_community.tools import PolygonAggregates
...
### Test 1: Ignore off-topic questions
...
def test_no_tools_on_offtopic_query(query: str) -> None:
    ...
    actual = result['messages'][1].tool_calls
...
def test_searches_for_correct_ticker() -> None:
    ...
    # Log the test example
    query = "What is the price of Aple?"
    t.log_inputs({"query": query})
    expected = "AAPL"
...
def test_executes_code_when_needed() -> None:
    query = "What was the average return rate for FAANG stock in 2024?"
    t.log_inputs({"query": query})
    expected = 53
    ...
    tool_calls = [tc['name'] for msg in result['messages'] for tc in getattr(msg, "tool_calls", [])]
    ...
    assert "code_tools" in tool_calls
...
class Grade(TypedDict):
    ...
    score: Annotated[bool, ..., "Return True if the answer is fully frounded in the source documents, otherwise False."]
...
def test_no_tools_on_offtopic_query(question: str) -> None:
    """Test that the agent does not use tools on offtopic queries."""
...
@pytest.mark.langsmith
def test_looks_up_correct_ticker():
    """Test that the agent gets the price of the correct ticker."""
    question = "What is the price of Aple?"
    ...
    for msg in result['messages'][1:]:
        if hasattr(msg, "tool_calls") and any(tc['name'] == )
    actual = result['messages'][1].tool_calls[0]['args']["query"]["ticker"]

    t.log_outputs({"ticker": actual})

    t.log_feedback(key="num_messages", score=len(result['messages']))
...
def test_grounded_in_the_source():
    ...
    with t.trace_feedback():
        ...
        t.log_feedback(key="groundedness", score=score)
    assert score`,
    },
    {
      value: "vitest",
