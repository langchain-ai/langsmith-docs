...
2. Add the selected experiments to an **annotation queue** where a human expert can add evaluator scores.
3. **Test your LLM-as-judge** against human feedback. Check the cases where your evaluator result is not aligned with the human expert. This indicates areas where your evaluator prompt needs improvement.
4. **Refine and repeat** to improve evaluator alignment. Update your LLM-as-judge prompt and test again.

:::note Offline evaluators support only
Currently this feature is only supported for evaluations over a dataset ([offline evaluations](/evaluation/concepts#offline-evaluation)), however we do plan to support this for [online evaluators](/evaluation/concepts#online-evaluation) as well in the near future. 
:::
...
## Aligning your evaluator

You can align both an existing evaluator and a new evaluator, using a similar process. 

### Align a new evaluator
...
6. Click **Select Experiments** and follow the steps below

### Align an existing evaluator

1. Navigate to the **Datasets and Experiments** tab
2. Select a dataset
3. Go to the **Evaluators** tab
4. Click on the evaluator you want to align
5. In the "Align Evaluator with experiment data" box, click **Select Experiments** and follow the steps below
