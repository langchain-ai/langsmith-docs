...
In order to track regressions, you need a baseline experiment against which to compare and a metric to measure.
The first experiment in your comparison will be used as the baseline by default, but you can change it from the dropdown at the top of the page.

You will also need to select which feedback key (evaluation metric) you want to focus on.
Again, one will be assigned by default, but you can adjust as needed.

You can configure whether a higher score is better for the selected feedback key. This preference will be stored.
