...
The JS/TS SDK has an analogous Jest/Vitest/Bun integration.
...
All you need to do is decorate the relevant tests with `@pytest.mark.langsmith`.
...
Runnig this test will create/update an example with name "test_foo", inputs `{"a": 1, "b": 2}`, reference outputs `{"foo": "bar"}` and trace a run with outputs `{"foo": "baz"}`.
...
By default LangSmith does not change the pytest terminal output. If you'd prefer, you can 
