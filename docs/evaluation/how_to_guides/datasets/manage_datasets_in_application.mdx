...
To create a dataset, head to the Datasets & Experiments page in LangSmith, and click `+ Dataset`. You'll have 3 options for
getting started:

TODO: INSERT IMAGE WHEN COPY FINAL

* **Create from CSV:** Drag and drop your CSV into LangSmith and your data will be imported automatically. Each row will correspond to a new dataset example.
* **Create from scratch:** If you prefer to upload data from the SDK or manually, you can instead set up your dataset before seeding any data. 
   ...
   Dataset schemas are defined with standard [JSON schema](https://json-schema.org/), with the addition of a few [custom types](/reference/data_formats/dataset_json_types) 
   that make it easier to define commonly used LLM data primitives like messages and tool definitions.

   As part of your schema definition, certain fields allow you to click `+ Transformations` which enable common
   preprocessing on your data to occur when you add examples to your dataset, such as converting LangSmith traces 
   into standard openAI message formats, or dropping fields that aren't defined in your schema. For the full list 
   of available transformations, see [our reference](/reference/data_formats/dataset_transformations)

* **Create from template**: If you plan to collect production traces in your dataset from LangChain 
[ChatModels](https://python.langchain.com/api_reference/core/language_models/langchain_core.language_models.chat_models.BaseChatModel.html) 
or from OpenAI calls using the [LangSmith OpenAI wrapper](/observability/how_to_guides/tracing/annotate_code#wrap-the-openai-client), we offer a prebuilt schema and transformation that makes it easy to
collect these runs in format that is easy to use downstream. This is entirely customizable, and does not lock your dataset into a set format. 

   Please see the [dataset transformations reference](/reference/data_formats/dataset_transformations) for more information. 
   
   :::note
   Users who keep their prompt in their system message often prefer to add the `Remove System Messages` transformation to 
   their dataset in addition to the default ones added by the template. This prevents the Sytem prompt, which is often 
   what people want to evaluate, from entering the dataset. 
...
### Add run(s) from the tracing project UI
...
There are two ways to add data from tracing projects to datasets. When runs are added from tracing projects to 
datasets, the inputs, outputs, and metadata of the run all carry over to the example in the dataset.
...
### Set up an automation to add a subset of all runs in a tracing project to a dataset
...
### Add run(s) from an annotation queue
...
As with adding directly from the run details page, any modifications you make to the run in your annotation queue will carry over to 
the dataset, and all metadata associated with the run will also be copied.

### Add runs directly via the Datasets UI

Go to the Datasets & Experiments UI, click the `Examples` tab, and `+ Example`. This will let you either define 
JSON inline

### Add synthetic examples created by an LLM via the Datasets UI

If you have a schema defined on your dataset, when clicking `+ Example` as in the previous section, you can decide to generate examples instead of manually input them. 
