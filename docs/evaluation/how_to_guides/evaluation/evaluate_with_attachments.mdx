...
You can also add examples with attachments from existing runs that contain attachments.
To do learn how to do so you can read [this guide](./../datasets/manage_datasets_in_application#add-runs-from-the-tracing-project-ui).
...
LangSmith also supports using the asynchronous version of the `evaluate` method, `aevaluate`, to run evaluations asynchronously:
...
### Use attachments in your evaluator
