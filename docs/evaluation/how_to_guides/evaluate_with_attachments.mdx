...
When evaluating models that work with multimodal content, you can incorporate attachments into your prompts through the LangSmith UI:

1. Click the file icon in the message where you want to add multimodal content
2. Choose between:
   - Inline content: Static content included in all prompts
   - Template variables: Dynamic content pulled from each example's attachments

To reference attachments in your prompts:
- For a single attachment type: Use the suggested variable name (all examples must have attachments with matching names)
- For multiple attachments or varying names: Use the `All attachments` variable to include all available attachments
