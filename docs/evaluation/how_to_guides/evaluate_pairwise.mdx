...
To do this, use the [evaluate()](https://docs.smith.langchain.com/reference/python/evaluation/langsmith.evaluation._runner.evaluate) function with two existing experiments.
...
<CodeTabs
  tabs={[
    ...
    typescript({caption: "Requires `langsmith>=0.2.9`"})`
      ...
      async function rankedPreference({ inputs, outputs }: { inputs: Record<string, any>, outputs: Record<string, any> }) {
