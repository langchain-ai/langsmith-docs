...
For example, if you're quickly iterating on a prompt and want to smoke test it on a few examples, or if you're validating out that your target and evaluator functions are all defined correctly, you may not want to record these evaluations.
...
<CodeTabs
  tabs={[
    python({caption: "Requires `langsmith>=0.1.145`. Example also uses `pandas`."})`
