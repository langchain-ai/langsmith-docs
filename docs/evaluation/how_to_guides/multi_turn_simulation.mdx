...
Some AI applications operate over multiple interactions with a user, also called conversation *turns*.
When evaluating the performance of such applications, core concepts such as [gathering a dataset](../../evaluation/concepts#datasets) and defining
[evaluators](../../evaluation/concepts#evaluators) to judge results successful remain useful - the inputs and outputs for these
cases become a trajectory of messages rather than a single one.

However, you may find it useful to instead run a *simulation* between your app and a user, then evaluate this dynamically
created trajectory. The advantages of this approach are that it doesn't require pre-existing trajectories and can test your app over a wider range
of natural language inputs. The disadvantage is that it is less consistent, as the increased number of turns and LLM calls introduces more randomness.
...
Congrats! You just ran your first multi-turn simulation. Next, we'll cover how to run it in LangSmith.
...
an `outputs` kwarg. Your passed `trajectory_evaluator` must therefore accept this kwarg. Here's an example:
...
const client = new wrapOpenAI(new OpenAI());
...
ls.describe("Multiturn demo", () => {
    ls.test(
      ...
      async ({ inputs }) => {
        ...
        ls.logOutputs(result.trajectory);
...
runner to evaluate simulated multi-turn interactions. This will look a little bit different from the `pytest`/`Vitest`/`Jest` example as follows:
...
- Instead of using the `trajectory_evaluators` param, you should pass your evaluators into `evaluate()`.
...
await langsmith.createExamples([{ inputs, datasetId: dataset.id }]);
...
const target = ({ inputs }: { inputs: Record<string, unknown> }) => {
    const history = {};
