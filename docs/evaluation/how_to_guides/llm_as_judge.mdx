...
Pre-built evaluators are a useful starting point setting up evaluations. Refer to [pre-built evaluators](./prebuilt_evaluators) for how to use pre-built evaluators with LangSmith.
...
See [here](./custom_evaluator) for more on how to write a custom evaluator.
  ...
  Pre-built evaluators easy to customize. By default, pre-built evaluators assume that you are running the evaluation over the full input, output and (if applicable) reference output. To specify which keys to run the evaluation over, or to change the LLM-as-a-judge prompt you can edit the evaluator. See more details below. 
...
Feedback is added as output schema to your LLM-as-a-judge evaluator prompt. If you are pulling in a prompt from the hub, you must add feedback configuration as part of the output schema for your prompt in the playground first.
