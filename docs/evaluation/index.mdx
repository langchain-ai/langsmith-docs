...
[`openevals`](https://github.com/langchain-ai/openevals) package, though you can also [write your own custom evaluators](./how_to_guides/custom_evaluator) if you desire.
...
<CodeTabs
  tabs={[
    ...
    },
    {
      ...
      label: "TypeScript",
      language: "bash",
      content: `yarn add langsmith openevals openai`,
    },
  ]}
...
We'll be using an OpenAI model under the hood, so you'll need to set the `OPENAI_API_KEY` environment variable as well as the
...
def correctness_evaluator(inputs: dict, outputs: dict, reference_outputs: dict):
    evaluator = create_llm_as_judge(
        # CORRECTNESS_PROMPT is just an f-string with
        # variables for "inputs", "outputs", and "reference_outputs"
    ...
    },
    {
      value: "typescript",
      label: "TypeScript",
      content: `import { createLLMAsJudge, CORRECTNESS_PROMPT } from "openevals";

const correctnessEvaluator = (params: {
  inputs: Record<string, unknown>,
  outputs: Record<string, unknown>,
  referenceOutputs: Record<string, unknown>
}) => {
  const evaluator = createLLMAsJudge({
    // CORRECTNESS_PROMPT is just an f-string
    prompt: CORRECTNESS_PROMPT,
    model: "openai:o3-mini",
  });
