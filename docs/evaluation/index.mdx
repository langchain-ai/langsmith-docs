...
<CodeTabs
  tabs={[
    python({caption: "Requires `langsmith>=0.1.145`"})`# Define the application logic you want to evaluate inside a target function
  ...
  tabs={[
    {
      value: "python",
      label: "Python",
      content: `# Define output schema for the LLM judge
...
<CodeTabs tabs={[
python`# After running the evaluation, a link will be provided to view the results in langsmith
