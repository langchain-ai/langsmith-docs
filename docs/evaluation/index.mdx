...
GRADE_ACCURACY_PROMPT = """You are a grader assessing a Student Answer relative to a reference Ground Truth answer.

Is the Student Answer conceptually grounded in and similar to the Ground Truth answer?

We do not expect all of the text in Student Answer to be identical to the Ground Truth answer.

The main point is that the same concept is employed.

A score of 1 means that the Student Answer answer is not at all conceptually grounded in and similar to the Ground Truth answer.

A score of 5 means that the Assistant answer contains some information that is conceptually grounded in and similar to the Ground Truth answer.

A score of 10 means that the Assistant answer is fully conceptually grounded in and similar to the Ground Truth answer.
...
def new_pipeline_answer(inputs):
    response = openai_client.chat.completions.create(
      ...
      messages=[
              {
                "role": "system", 
                "content": "Answer the following question accurately and concisely. Use emojis to make the response more engaging and fun."
              },
              {
                "role": "user", 
...
// Define accuracy evaluation prompt
const GRADE_ACCURACY_PROMPT = \`You are a grader assessing a Student Answer relative to a reference Ground Truth answer.
Is the Student Answer conceptually grounded in and similar to the Ground Truth answer?
...
A score of 1 means that the Student Answer is not at all conceptually grounded in and similar to the Ground Truth answer.
...
role: "system",
content:
"Answer the following question accurately and concisely. Use emojis to make the response more engaging and fun.",
},
...
`,
    typescript`
// Name the experiment
const experimentPrefix = "first-eval-in-langsmith";
