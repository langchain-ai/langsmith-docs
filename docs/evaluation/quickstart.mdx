...
LangSmith helps you evaluate your AI system over real-world data so you can deploy the best perfoming model for your needs. This guide gets you started with the basics.

Start by installing LangSmith. If you want to use pre-built [LangChain evaluators](https://python.langchain.com/docs/guides/evaluation/), you can install LangChain as well.
...
<CodeTabs
    tabs={[
        ...
        label: "Python",
        language: "bash",
        content: `pip install -U langsmith\npip install -U langchain # If you want to use pre-built open source evaluators`,
        },
        {
        value: "typescript",
        label: "TypeScript",
...
The [dataset](faq) defines test cases over inputs your pipeline should handle well.
...
The `evaluate` test runner expects a function that accepts a dictionary (the `example.inputs` from your dataset) and should return a dictionary (containing the predicted outputs).
...
<p>The <code>evaluate()</code> method calls your `predict()` function on each example in the dataset. It traces this process and provides the traces and examples to your <a href="faq/custom-evaluators">evaluators</a>, which return scores for each prediction.</p>
