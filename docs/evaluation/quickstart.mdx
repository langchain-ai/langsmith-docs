...
tabs={[
    ...
    language: "python",
    content: `# You evaluate any arbitrary function over the dataset.
...
tabs={[
    {
        ...
        content: `// You evaluate any arbitrary function over the dataset.
// The input to the function will be the inputs dictionary for each example.
async function predictResult(input_: Record<string, any>): Record<string, any> {
    return {output: "Bar Bar Bar"};
}`,
    },
    {
...
For more information on evaluators you can use off-the-shelf, check out the [pre-built evaluators](faq) docs or the [reference documentation](https://api.python.langchain.com/en/latest/api_reference.html#module-langchain.evaluation) for LangChain's evalution module.
For more information on how to write a custom evaluator, check out the [custom evaluators](faq) guide.
...
To learn more about evaluation chains available out of the box, check out the [LangChain Evaluators](faq) guide. To learn how to make your own custom evaluators, review the [Custom Evaluator](faq) guide.
