...
To learn more about when to use workspaces, read more [here](./concepts#workspaces). By default, the PAT will target the workspace it was created in. To learn more about when  To manage resources in a workspace different from the default, you can specify the desired workspace ID in the client declaration. The sample code
