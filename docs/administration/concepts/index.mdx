...
Expiration dates can be set for all new API keys. The created key will become unusable after the amount of time determined by its creator. Keys can also be set to never expire, and keys created before the implementation of this feature will automatically be set to never expire.
