...
Service keys are similar to PATs, but are used to authenticate requests to the LangSmith API on behalf of a service account. We recommend using these for applications / services that need to interact with the LangSmith API, such as LangGraph agents or other integrations.
Service keys are scoped to a workspace and can be used to authenticate requests to the LangSmith API for that workspace. If the service key is deleted, it will no longer work.
