...
async def test_code_block_3():
    ...
    chain = LLMChain.from_string(
        llm=chat_model, template="What's the answer to {input}?"
    )
...
async def test_code_block_6():
    ...
    with trace_as_chain_group("my_group") as group_manager:
        chain.invoke(
            {"question": "What is your name?"}, config={"callbacks": group_manager}
        )
        chain.invoke(
            {"question": "What is your quest?"}, config={"callbacks": group_manager}
        )
        chain.invoke(
...
async def test_code_block_8():
    ...
    callbacks = [
        LangChainTracer(
            ...
            client=Client(
                api_url="https://api.smith.langchain.com", api_key="YOUR_API_KEY_HERE"
            ),
        )
    ]
